...
show_only_frames = [0, 200] # show only frames in interval for debugging; 200 originially

## Prepare Waymo Open Dataset file for loading
img_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'img/')
movie_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'movie/')
...
while True:
    try:
        ...
        if 'show_objects_in_bev_labels_in_camera' in exec_list:
            tools.show_objects_in_bev_labels_in_camera(detections, lidar_bev, image, frame.laser_labels, valid_label_flags, camera_calibration, configs_det, cnt_frame)
            cv2.waitKey(vis_pause_time)               
